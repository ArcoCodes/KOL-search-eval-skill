...
Agent 按 `references/rough-eval-rules.md` 综合判断，重点筛注水/买量号。核心看：

- **互动比例失真**：点赞率异常高（YouTube >8%）、赞多评少（评论/赞比 <0.5%）、赞评跨视频相关性低（<0.3）
- **粉播倒挂**：粉丝多但中位播放极少
- **评论灌水**：模板化重复/泛泛夸赞占比过高
- **基本面**：活跃度（>90天淘汰）、粉丝最低线（YouTube/TikTok/Instagram ≥1K，Twitter ≥500）

**直接淘汰**：多个注水信号同时命中 / 评论质量=严重灌水 / 粉播严重倒挂 / 不活跃

Agent 给出综合评分 0-100（自主判断，非维度分加总），**通过线**：≥60 分
...
候选池独立存储关键字段（名称/主页URL/平台内ID/粉丝/ER/均播放/受众辐射/评论质量/粗估得分(Agent综合判断)/粗估依据）。
...
```
Step 1              Step 2              Step 3                         Step 4
搜索                信号采集            Agent + write_candidate.py      write_kol.py
