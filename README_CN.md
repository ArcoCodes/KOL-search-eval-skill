...
KOL-search-eval-skill 是一个用于创作者发现、信号采集、粗估筛选和细估评价的skill，预计支持 YouTube、TikTok、Instagram 和 Twitter/X 四个平台（目前版本只支持youtube）。
...
安装 `lark-cli` 后，首次使用需要完成飞书 OAuth 授权：

```bash
lark-cli auth login --domain base
```

终端会显示一个验证链接，在浏览器中打开并用飞书账号授权即可。`lark-cli` 内置了共用的应用凭证，不需要自己创建飞书应用。
...
- `lark-cli` 认证过期：运行 `lark-cli auth login --domain base` 重新授权。
